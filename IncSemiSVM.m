classdef IncSemiSVM
    ...
    methods(Static = true)
        ...
        function [model,Lambda]=Run(original_x,original_y,initial_solution)
            global C lambda_max t0 fake_zero2 lambda
            svm_struct=initial_solution;
            model=[];
            Lambda=[];
       
            Q=SMO.Kernel(original_x,svm_struct.x).*(original_y*ones(1,length(svm_struct.y)));
            local_Q=[original_y Q];
            local_alpha=[svm_struct.b;svm_struct.alpha];
            local_f=local_Q*local_alpha;
            Index_P = find(local_f<1-lambda);
            P_fx = local_f(Index_P);
            while (lambda_max>lambda && ~isempty(Index_P))
                lambda = 1-min(P_fx);
                Index_Q = find(P_fx==1-lambda);
                Q_x = svm_struct.x(Index_Q,:);
                Q_y = svm_struct.y(Index_Q);
                % Incremental and Decremental
%                 while length(Q_y)>0
%                     g=IncSemiSVM.GetG(original_x,original_y,svm_struct);
%                     [svm_struct, index_A, direction_A]=IncSemiSVM.InitialA(svm_struct,g(Index_Q,length(Q_y),Q_y);
%                     [Qc]=IncSemiSVM.GetQc(svm_struct,direction,o);
...
%                     Q_y = index_A;
%                 end
                svm_struct = initial(original_x,original_y);
                Q=SMO.Kernel(original_x,svm_struct.x).*(original_y*ones(1,length(svm_struct.y)));
                local_Q=[original_y Q];
                local_alpha=[svm_struct.b;svm_struct.alpha];
                local_f=local_Q*local_alpha;
        ...
        function  [out_svm_struct,direction_A,index_A] = UpdataSvmstruct...
            ...
            global  fake_zero size_training C_star num data_flag
            local_beta=beta;
...
%                     return;
        ...
        function  [max_delta_zeta,index,index_class]=GetMaxChange...
            ...
            if index_max_change_alpha == inf
                k=1;
            end
            if index_max_change_g == inf
                k=2;
            end
            if index_max_change_A == inf
                k=3;
            end
            if index_max_change_toA == inf
                k=4;
            end
